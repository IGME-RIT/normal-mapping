...
struct VertexFormat
{
	...
	glm::vec3 normal;	// A vector perpendicular to the surface
	glm::vec3 tangent;	// This is used along with the normal map to come up with 3 axis with normal heignt one of them.

	// Constructor
	VertexFormat(const glm::vec2 &texCo, const glm::vec3 &pos, const glm::vec3 norm, const glm::vec3 tang)
	{
		texCoord = texCo;
		position = pos;
		normal = norm;
...
void init()
{
	...
	//Since we are sending multiple textures, we need to define which sampler we are loading into.
	// We can pass 8 textures to a shader at any given time.
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex[0]);

    const char *fileLoc = "brickTexture.png";
	...
	const char *file2Loc = "brickNorm.png";
	bitmap = FreeImage_Load(
		FreeImage_GetFileType(file2Loc, 0),
		file2Loc);
	...
	trans = trans = glm::rotate(trans, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	PV = proj * view;
...
void setup()
{
	...
	//Plane Face
	vertices.push_back(VertexFormat(glm::vec2(1.0, 1.0), glm::vec3(1.0, -1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));	// bottom right
	vertices.push_back(VertexFormat(glm::vec2(0.0, 1.0), glm::vec3(-1.0, -1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));// bottom left
	vertices.push_back(VertexFormat(glm::vec2(1.0, 0.0), glm::vec3(1.0, 1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));	// top right

	vertices.push_back(VertexFormat(glm::vec2(1.0, 0.0), glm::vec3(1.0, 1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));	// top right
	vertices.push_back(VertexFormat(glm::vec2(0.0, 1.0), glm::vec3(-1.0, -1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));// bottom left
	vertices.push_back(VertexFormat(glm::vec2(0.0, 0.0), glm::vec3(-1.0, 1.0, 0), glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)));	// top left
	...
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)20);

	glEnableVertexAttribArray(3);
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)32);
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Normal Mapping", nullptr, nullptr);
